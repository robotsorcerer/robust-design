...
__author__ 		= "Giuseppe Armenise, Lekan Molu"
__copyright__ 	= "2022, Robust Learning."
__license__ 	= "Microsoft License"
__comment__     = "An ARMAX model estimator."
...
class Armax(object):
    ...
    def _identify(y, u, na, nb, nc, delay, max_iterations):
        ...
        while (Vn_old > Vn or iterations == 0) and iterations < max_iterations:
            ...
            Vn = self.mean_square_error(y[max_order::], np.dot(X, beta_hat))
    ...
    def rescale(self, y):
        """
        Rescale an array to its standard deviation.
           :: y=y/(standard deviation of y)
           ::std: ex [Ystd,Y]=rescale(Y)
        """
        ystd = np.std(y)
        y_scaled = np.divide(y, ystd)
        return ystd, y_scaled

    def information_criterion(self, K, N, Variance, method='AIC'):
    ...
    def find_best_estimate(self, y, u):
        ...
        y_std, y = self.rescale(y)
        u_std, u = self.rescale(u)
            ...
            for nb in self.nb_range:
                for nc in self.nc_range:
                    for delay in self.delay_range:
                        ...
                        IC = self.information_criterion(na + nb + delay,
